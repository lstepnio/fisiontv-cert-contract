...
floor (e.g., a known-broken release has been pulled). The client logs
the response and **still runs the cert** — the result will carry the
old `versionCode` and the server is free to flag it post-hoc. The 426
is a signal to escalate the auto-update urgency (the client retries
the manifest fetch sooner next session), not a cert-blocking gate.
